...
#### CONSTANTS
ICCD_IMAGE_SIZE = (
    1024  # Number of rows in ICCD image, also number of columns in ICCD image
)
...
DEF_BG_ROI = np.ix_(
    np.append(np.arange(100, 300), np.arange(700, 900)), np.arange(500, 700)
)
#### END CONSTANTS
...
def get_spectra(run, bg_img, cals):
    ...
    for current_img in iccd_images:
        ...
        no_sam_spec, sam_spec = get_spectra_from_image(
            spec_image, cals["shear"], cals["no_sam_spec_ROI"], cals["sam_spec_ROI"]
        )
        no_sam_specs.append(no_sam_spec)
        sam_specs.append(sam_spec)
    data_dict = {
        "sam_spec": np.array(sam_specs),
        "no_sam_spec": np.array(no_sam_specs),
        "run_num": np.array([run] * len(iccd_images)),
        "image_num": np.arange(len(iccd_images)),
    }
    dtype_args = [
        (key, data_dict[key].dtype.type, np.size(data_dict[key][0]))
...
def shear_image(shear, imgIn, firstRowIndex=0):
    ...
    def shear_func(output_coords):
        ...
        input_coords = (
            output_coords[0],
            output_coords[1]
            + (output_coords[0] + firstRowIndex - ICCD_IMAGE_SIZE) * shear,
        )
        return input_coords
