...
def quant():
    ...
    axs[0].scatter(measured['short_fluences']*1E-12, measured['short_efficiencies'], color='k', label='Expt.')
    axs[1].scatter(measured['long_fluences']*1E-12, measured['long_efficiencies'], color='k', label='Expt.')
    axs[0].plot(sim_results_5fs['fluences']*1E3, np.array(sim_results_5fs['stim_efficiencies']), color='tab:blue', label='Three Level\nSimulation')
    axs[1].plot(sim_results_25fs['fluences']*1E3, np.array(sim_results_25fs['stim_efficiencies']), color='tab:blue', label='Three Level\nSimulation')
    axs[0].plot(markus['5fs']['fluence'], markus['5fs']['stim']*100, color='tab:orange', label='Rate Eqs.')
    axs[1].plot(markus['25fs']['fluence']*5, markus['25fs']['stim']*100, color='tab:orange', label='Rate Eqs.')
    format_quant_plot(axs)
    plt.savefig('plots/2020_09_02_quant.eps', dpi=600)
    plt.savefig('plots/2020_09_02_quant.png', dpi=600)
...
def format_quant_plot(axs):
    ...
    axs[0].text(0.9, 0.1, 'a', fontsize=10, weight='bold', horizontalalignment='center', transform=axs[0].transAxes)
    axs[1].text(0.1, 0.9, 'b', fontsize=10, weight='bold', horizontalalignment='center', transform=axs[1].transAxes)
    axs[0].text(0.5, 0.1, '5 fs Pulses', fontsize=10, weight='bold', horizontalalignment='center', transform=axs[0].transAxes)
    axs[1].text(0.5, 0.9, '25 fs Pulses', fontsize=10, weight='bold', horizontalalignment='center', transform=axs[1].transAxes)
