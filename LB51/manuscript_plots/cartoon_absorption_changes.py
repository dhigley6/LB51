...
def make_linear_plot():
    ...
    ax.plot(
        sum_data359["phot"],
        sum_data359["ssrl_absorption"] - 0.6,
        label="Absorption",
        color="k",
    )
    format_cartoon()
...
def make_temp_change_plot():
    ...
    plt.plot(
        sum_data359["phot"],
        sum_data359["ssrl_absorption"] + abs_change - 0.6,
        color="k",
        label="Current State Absorption",
    )
    format_cartoon()
...
def fermi_dirac_change(phot, mu, T, T0=300.0, broad=0.43):
    ...
    sample_phot = np.arange(
        -21 * broad + np.amin(phot), 21 * broad + np.amax(phot) + diff / 10, diff / 10
    )
    ...
    gauss_broad_points = np.arange(
        -20 * broad,
        20 * broad + sample_phot[1] - sample_phot[0],
        sample_phot[1] - sample_phot[0],
    )
