...
def load_markus_simulations():
    markus_5fs = np.genfromtxt(
        "../data/proc/Markus_5fs.txt", delimiter="", skip_header=1
    )
    markus_25fs = np.genfromtxt(
        "../data/proc/Markus_25fs.txt", delimiter=" ", skip_header=1
    )
    return {"5": markus_5fs, "25": markus_25fs}
...
def plot():
    ...
    plt.scatter(
        measured["short_fluences"] / 5,
        measured["short_efficiencies"],
        label="5 fs Pulses\nExpt.",
    )
    plt.scatter(
        measured["long_fluences"] / 25,
        measured["long_efficiencies"],
        label="25 fs Pulses\nExpt.",
    )
    plt.semilogx(
        strengths * 1e15,
        np.array(stim_efficiencies) * 100,
        color="k",
        label="Three Level\nSimulation",
    )
