...
def fit_abs_copd(
    sam_spec, no_sam_spec, splitting_guess=1, L3_guess=535, ev_per_pixel_guess=0.07
):
    copd_xas = LB51_get_synchro.get_copd_xas()
    fit_coef_dict = fit_abs(
        sam_spec,
        no_sam_spec,
        copd_xas,
...
def fit_abs_plain_coo(
    sam_spec,
    no_sam_spec,
    thick_ratio_guess=1.5,
    splitting_guess=1,
    L3_guess=540,
    ev_per_pixel_guess=0.07,
):
    """Get Plain CoO cals"""
...
def fit_abs_np_coo(
    sam_spec, no_sam_spec, splitting_guess=1, L3_guess=540, ev_per_pixel_guess=0.07
):
    """Get NP CoO cals"""
    np_coo_xas = LB51_get_synchro.get_np_coo_xas()
    fit_coef_dict = fit_abs(
...
def fit_abs(
    ...
    guesses = (thick_ratio_guess, splitting_guess, L3_guess, ev_per_pixel_guess)
    ...
    start_params = (thick_ratio_guess, splitting_guess, L3_guess, ev_per_pixel_guess)
    start_params = [
        start_param for start_param in start_params if start_param is not None
    ]
    ...
    axs[0].plot(
        phot, no_sam_spec * fit_coef_dict["splitting"], "k--", label="Incident Spectra"
    )
    est_sam_spec = get_est_sam_spec(fit_coef)
    ...
    axs[1].plot(
        phot,
        -np.log(sam_spec / no_sam_spec / fit_coef_dict["splitting"]),
        label="LCLS Absorption",
    )
...
def pixel_to_photon_e(pixels, cal_data):
    ...
    pix_energy = (pixels - cal_data["L3_pixel"]) * cal_data[
        "ev_per_pixel"
    ] + Co_L3EDGE_MAX
    return pix_energy
