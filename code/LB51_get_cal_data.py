...
def combine_burst_data(data_sets):
    ...
    sum_intact = get_summed_spectra(data_intact['sam_spec'], data_intact['no_sam_spec'], cals)
    sum_intact['fluence'] = np.mean(data_intact['fluence'])
    sum_blown = get_summed_spectra(data_blown['sam_spec'], data_blown['no_sam_spec'], cals, blown_sam=True)
    to_ret = {'total': data_total,
              'intact': data_intact,
              'blown': data_blown,
...
def get_total_burst_data(run, correct_split=True):
    ...
    data = {'fluence': get_fluences(nonspec_data['gd2'], cals),
...
def get_nonburst_data(run):
    ...
    # Assemble data into numpy record array
    data_intact = {'sam_spec': spec_data['sam_spec'],
                   'no_sam_spec': spec_data['no_sam_spec'],
                   'run_num': spec_data['run_num'],
                   'image_num': spec_data['image_num']}
    data_intact = dict_to_rec_array(data_intact)
    sum_intact = get_summed_spectra(spec_data['sam_spec'], spec_data['no_sam_spec'], cals)
    fluence = get_fluences(nonspec_data['gd2'], cals)
    sum_intact['fluence'] = np.mean(fluence)
    to_ret = {'intact': data_intact,
              'sum_intact': sum_intact,
              'nonspec': nonspec_data}
    return to_ret
...
def dict_to_rec_array(dict_in):
    ...
    """
    dtype_args = [(key, type(dict_in[key][0]), np.size(dict_in[key][0])) for key in dict_in.keys()]
