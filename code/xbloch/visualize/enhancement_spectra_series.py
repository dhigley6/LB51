...
def make_figure(strengths, sim_results):
    ...
    linear_difference = (np.abs(linear_case['E_out']**2)-np.abs(linear_case['E_in']**2))/strengths[0]
    for i, sim_result in enumerate(sim_results):
        ...
        spec_out = 2*np.abs(phot_result['E_in'])**2/strengths[i]+diff_from_linear*enhancement_factors[i]+linear_difference
        axs[1].plot(phot_result['phots'], spec_out)
        axs[2].plot(phot_result['phots'], diff_from_linear*enhancement_factors[i]+linear_difference)
    _format_figure(axs)
